...
static packet_t *packetize(char *buffer, int length, int *count) {
    ...
    num = (length % MAX_PAYLOAD_LENGTH) ? num + 1 : num;
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (i + 1 < num) {
            ...
            max = (max == 0) ? MAX_PAYLOAD_LENGTH : max;
            packet.payload_length = max;
        }
        for (int x = 0; x < packet.payload_length; x++) {
            packet.payload[x] = buffer[(i * MAX_PAYLOAD_LENGTH) + x];
        }
